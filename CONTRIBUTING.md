...
| --- | --- |
| `Sources/SwitchboardCore` | Sessions, storage, switching, the cache catalogue. No SwiftUI, and nothing that needs a window. |
...
- **Tests for anything that decides something.** Tests in this repository run
  ...
  `Tests/SwitchboardCoreTests/Support.swift`. No test should touch the real
...
- **No new dependencies** without a reason in the pull request description.
  Switchboard depends on nothing but the system frameworks today, which is a
  large part of why it is auditable.

## Adding a cache target

This is the easiest useful contribution. Add an entry to
`Sources/SwitchboardCore/CacheCatalog.swift`:
