...
[![CI](https://github.com/RamitVishwakarma/Switchboard/actions/workflows/ci.yml/badge.svg)](https://github.com/RamitVishwakarma/Switchboard/actions/workflows/ci.yml)
[![Latest release](https://img.shields.io/github/v/release/RamitVishwakarma/Switchboard?sort=semver)](https://github.com/RamitVishwakarma/Switchboard/releases/latest)
...
- **Switches Claude Code accounts.** Signing in as another account normally means
  signing out first and typing the whole thing again. Switchboard saves each
...
[Releases](https://github.com/RamitVishwakarma/Switchboard/releases/latest), open
it, and drag Switchboard to Applications. The build is universal — Apple silicon
and Intel.
...
osascript -e 'tell application "System Events" to make login item \
  at end with properties {path:"/Applications/Switchboard.app", hidden:true}'
```
...
```sh
git clone https://github.com/RamitVishwakarma/Switchboard.git
cd Switchboard
./build.sh
...
Switchboard cannot sign in for you, so the first account has to be signed in
...
with how much of their five-hour and weekly limits each has spent — useful when
the decision you are actually making is *which account has room left*.
...
Switching is just moving that pair. Switchboard copies the live pair into storage
under the account it belongs to, then writes the other account's saved pair into
place. Saved tokens go into the login keychain under the service `Switchboard`;
saved settings go to `~/Library/Application Support/Switchboard/`, readable only
by you.
...
- **The settings file is written back whole.** It belongs to another program and
  gains keys between releases, so Switchboard parses what it needs and preserves
...
a declined keychain prompt, a missing saved session — the Mac is left signed into
the account it was already signed into.

Signing in outside the app is handled too. If the live account is not one
Switchboard knows about, it is saved as a new account rather than overwritten,
...
- **Only caches inside your home directory can be touched.** Anything outside it
  is refused, as are the directories everything else lives inside — `~/Desktop`,
  `~/Library`, `~/.ssh`, `~/.claude` and others. This is enforced in code, not by
  being careful when editing the list, and there is a test asserting every entry
  in the catalogue passes it.
...
Some paths — `~/Downloads`, `~/.Trash` — are behind macOS privacy controls. Grant
Full Disk Access in System Settings → Privacy & Security if you want them covered.

### Adding a cache to the list

Add an entry to `CacheEntry.developerTools` or `CacheEntry.applications` in
[`Sources/SwitchboardCore/CacheCatalog.swift`](Sources/SwitchboardCore/CacheCatalog.swift):
...
./build.sh           # assemble Switchboard.app
```
...
| --- | --- |
| `SwitchboardCore` | Sessions, storage, the switch itself, the cache catalogue and its safety rules. No SwiftUI. |
| `Switchboard` | The SwiftUI window, the menu bar item, and the models behind them. |
| `SwitchboardCoreTests` | Everything in `SwitchboardCore`, against a temporary home directory and an in-memory keychain. |
...
Switchboard is not affiliated with, endorsed by, or supported by Anthropic. It
